...
process_file() {
    ...
    if [[ -d "$file" ]]; then
        ...
        if [[ $(find "$file" -mmin +$(($timeout * 60))) ]]; then
            ...
            echo "File $file is not old enough to delete"
        fi
    elif is_compressed "$file"; then
        # If file is compressed
        echo "Renaming and touching compressed file: $file"
        mv "$file" "fc-${file##*/}"
        touch "fc-${file##*/}"
    else
        # If file is not compressed
        echo "Compressing and removing file: $file"
...
# Remove fc-* files older than timeout
find . -type f -name 'fc-*' -mmin +$(($timeout * 60)) -exec echo "Deleting old file: {}" \; -exec rm -f {} \;
